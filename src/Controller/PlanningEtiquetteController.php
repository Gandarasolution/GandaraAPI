...
class PlanningEtiquetteController extends AbstractController
{
    ...
    {
        if ($idRessource <= 0) {
            return $this->json(['message' => 'ID de ressource invalide'], 400);
        }
    ...
    public function delete(int $idEtiquette): JsonResponse {
        if ($idEtiquette <= 0) {
            return $this->json(['message' => 'ID d\'étiquette invalide'], 400);
        }
        $result = $this->repository->deleteEtiquette($idEtiquette);
        if ($result === false) {
            return $this->json(['message' => 'Étiquette non trouvée ou déjà supprimée'], 404);
        }
        return $this->json(['message' => 'Étiquette supprimée avec succès']);
